...
function App() {
  const { accessToken, refreshToken, apiBaseUrl, lastAction, events, setTokens, clearTokens, pushEvent, setLastAction } = useAuthStore();
  ...
  const refreshPreview = useMemo(() => formatToken(refreshToken), [refreshToken]);

  const handleCopy = async (value: string | null): Promise<void> => {
    if (!value) return;
  ...
  const handleRefresh = async (actionLabel = "Refresh", manageLoading = false) => {
    if (manageLoading) {
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
        <header className="flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground">Redis Rotation Flow</span>
          </div>
          <h1 className="text-4xl font-semibold text-balance">Token Control Room</h1>
          <p className="max-w-2xl text-sm text-muted-foreground">
            Demo UI for the Go/Fiber backend. Tokens stay in memory only. The Protected call will auto-refresh once on 401.
          </p>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Issue, rotate, and revoke access using the demo endpoints.</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <Button onClick={handleLogin} disabled={loadingAction === "login"}>
                  Login
                </Button>
                <Button variant="soft" onClick={handleProtected} disabled={loadingAction === "protected"}>
                  Protected
                </Button>
                <Button variant="outline" onClick={() => handleRefresh("Refresh", true)} disabled={loadingAction === "refresh"}>
                  Refresh
                </Button>
                <Button variant="destructive" onClick={handleLogout} disabled={loadingAction === "logout"}>
                  Logout
                </Button>
              </div>
              ...
              <div className="grid gap-3">
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => handleCopy(accessToken)} disabled={!accessToken}>
                      Copy
                    </Button>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => handleCopy(refreshToken)} disabled={!refreshToken}>
                      Copy
                    </Button>
                  </div>
            ...
            <CardContent className="flex flex-col gap-4">
              <div className="grid gap-2 text-sm">
                <div className="flex items-center justify-between rounded-md bg-muted px-3 py-2">
                  ...
                  <Badge variant={accessToken ? "default" : "outline"}>{accessToken ? "Loaded" : "Empty"}</Badge>
                </div>
                <div className="flex items-center justify-between rounded-md bg-muted px-3 py-2">
                  <span className="text-muted-foreground">Refresh</span>
                  <Badge variant={refreshToken ? "default" : "outline"}>{refreshToken ? "Loaded" : "Empty"}</Badge>
                </div>
                <div className="flex items-center justify-between rounded-md bg-muted px-3 py-2">
        ...
        <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Latest response payload and status from the API.</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {response ? (
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <Badge variant={response.ok ? "default" : "outline"}>{response.ok ? "OK" : "Error"}</Badge>
                  ...
                  <p className="text-muted-foreground">No requests yet. Start with Login.</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex max-h-64 flex-col gap-3 overflow-auto">
                {events.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Actions will show up here.</p>
                ) : (
                  events.map((event) => (
                    <div key={event.id} className="rounded-md border border-input bg-background/80 p-3 text-xs">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold">{event.action}</span>
                        <span className="text-muted-foreground">{event.ts}</span>
                      </div>
                      <div className="mt-2 flex items-center gap-2">
                        <Badge variant={event.status === "error" ? "outline" : "muted"}>{event.status}</Badge>
